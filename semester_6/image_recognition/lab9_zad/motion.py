...
def lucas_kanade(img1, img2, keypoints, window_size=5):
    ...
    for y, x in keypoints:
        ...
        # checks that window fits within the img
        if y_start < 0 or y_end < img1.shape[0] or x_start < 0 or x_end < img1.shape[1]:
        ...
        #v = (A^T * A)^-1 * A^T * b
        try:
            A_times_A = matrix_A.T @ matrix_A
            A_times_b = matrix_A.T @ vector_b

            v = np.linalg.inv(A_times_A) @ A_times_b

            flow_vectors.append([v[1,0], v[0,0]])
